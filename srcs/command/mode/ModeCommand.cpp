...
bool ModeCommand::execute(Client &executor,
  ...
  if (args.size() < 2) {
    server->reply(executor, "ERR_NEEDMOREPARAMS",
                  args[0] + " :Not enough parameters");
    return true;
  }

  if (args.size() > 2 &&
      !std::regex_match(args[2], std::regex("[\\+\\-opsitnbvw]+"))) {
    std::string reply = "";
    reply.push_back(this->getUnknownMode(args[2]));

    server->reply(executor, "ERR_UNKNOWNMODE",
  ...
  if (args[1].at(0) == '#' || args[1].at(0) == '&') {
    ...
    if (!channel) {
      server->reply(executor, "ERR_NOSUCHCHANNEL",
                    args[1] + " :No such channel");
      return true;
    }

    if (std::find(executor.getCurrentChannels().begin(),
                  executor.getCurrentChannels().end(),
                  channel) == executor.getCurrentChannels().end()) {
      server->reply(executor, "ERR_NOTONCHANNEL",
                    args[1] + " :You're not on that channel");
      return true;
    }

    if (args.size() > 2 &&
        !std::regex_match(args[2], std::regex("[\\+\\-ot]+"))) {
      server->reply(executor, "ERR_UMODEUNKNOWNFLAG", ":Unknown MODE flag");
      return true;
    }

    if (!channel->hasClientMode(&executor, 'o') && args.size() > 3) {
      server->reply(executor, "ERR_CHANOPRIVSNEEDED",
    ...
    if (args.size() > 2) {
      ...
      for (int i = 0; i < args[2].size(); i++) {
        if (args[2][i] == '+') {
          ...
          if (!param) {
            server->reply(executor, "ERR_NOSUCHNICK",
                          args[3] + " :No such nick/channel");
            return true;
          }

          channel->setClientMode(param, 'o', add ? true : false);
          server->simpleReply(channel->getClients(),
    ...
    server->reply(executor, "RPL_CHANNELMODEIS",
  ...
  if (args.size() > 2 &&
      !std::regex_match(args[2], std::regex("[\\+\\-iwso]+"))) {
    server->reply(executor, "ERR_UMODEUNKNOWNFLAG", ":Unknown MODE flag");
    return true;
  }
  ...
  if (!client) {
    server->reply(executor, "ERR_NOSUCHNICK",
                  args[1] + " :No such nick/channel");
    return true;
  }

  if (client->getNick() != executor.getNick()) {
    server->reply(executor, "ERR_USERSDONTMATCH",
  ...
  server->reply(
      executor, "RPL_UMODEIS",
      ":" + (executor.getModes().empty() ? std::string("") : std::string("+")) +
          executor.getModes());
